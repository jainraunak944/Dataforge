...
function card(s,x,y,w,h,title,items,color=BLUE,num=null,fs=9.5){
  ...
  const paras=[]; items.forEach((it,i)=>{ const r=rich(it).filter(p=>p.text!==''); r[0].options.bullet={indent:9}; r[0].options.paraSpaceAfter=3; r[r.length-1].options.breakLine = i<items.length-1; paras.push(...r); });
  s.addText(paras,{x:x+0.12,y:y+0.5,w:w-0.24,h:h-0.58,fontFace:BF,fontSize:fs,color:BODY,valign:'top',margin:0,isTextBox:true});
}
